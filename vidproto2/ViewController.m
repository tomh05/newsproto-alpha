...
/*-(void) setCaptions:(NSMutableArray *)caps{
   NSLog(@"set, %@",caps);
   self.captions = caps;
}
*/
...
- (void)initVideo {
   NSLog(@"vid %@",self.vidFilename.stringByDeletingPathExtension);
      NSLog(@"vid %@",self.vidFilename.pathExtension);
   NSURL *vidUrl = [[NSBundle mainBundle] URLForResource:self.vidFilename.stringByDeletingPathExtension withExtension:self.vidFilename.pathExtension];
   NSLog(@"vidurl %@",vidUrl.path);
